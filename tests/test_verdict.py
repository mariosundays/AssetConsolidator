...
cases = [
 # (path, exists, expect_recommend, expect_reason)
...
for path, exists, want_rec, want_reason in cases:
    rec, reason = ac.verdict(path, ROOT, exists)
    label = path[:52]
    check(label+" [rec]", rec, want_rec)
    check(label+" [why]", reason, want_reason)
...
for r in [ac.PICK_MISSING, ac.PICK_VOLATILE, ac.PICK_NETWORK,
    ...
    check(f"help {r}", r in ac.REASON_HELP, True)

print("\n=== missing never recommended, regardless of location ===")
for path in ["E:/lib/a.exr","//nas/x.exr","C:/temp/y.exr",ROOT+"/z.exr"]:
    rec,reason = ac.verdict(path, ROOT, False)
    check(f"missing {path[:20]}", (rec,reason), (False, ac.PICK_MISSING))

print("\n=== volatile beats drive check (temp on same drive) ===")
rec,reason = ac.verdict("F:/temp/scratch.exr", ROOT, True)
check("same-drive temp -> volatile", reason, ac.PICK_VOLATILE)
...
check("reason recorded", r.reason, ac.PICK_MISSING)  # file does not exist
